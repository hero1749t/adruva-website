...
import { Mail, Send, FileText, Briefcase, ChevronRight, Clock } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const { data: newsletterData, isLoading: newsletterLoading } = useQuery({
    ...
    queryFn: () => apiFetch<PaginatedResponse<unknown>>('/newsletter/subscribers?limit=1'),
  });

  const { data: blogsData, isLoading: blogsLoading } = useQuery({
    queryKey: ['admin', 'blogs', { limit: 1, status: 'all' }],
    queryFn: () => apiFetch<PaginatedResponse<unknown>>('/blog?limit=1&status=all'),
  });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <Card key={idx} className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#151f32] overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
            <CardContent className="p-6 relative">
              <div className="flex items-center justify-between">
                ...
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${stat.color} flex items-center justify-center text-white shadow-lg shadow-slate-200 dark:shadow-none`}>
      ...
      <Card className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#151f32] shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-slate-800/60">
          <div className="space-y-0.5">
            <CardTitle className="text-lg font-bold font-poppins text-slate-900 dark:text-white">Recent Inquiries</CardTitle>
            <CardDescription className="text-slate-500 dark:text-slate-400 text-xs font-inter">The latest incoming contact form submissions</CardDescription>
          </div>
          <Link
              ...
              <table className="w-full text-left border-collapse text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40 text-slate-700 dark:text-slate-300">
                  {inquiriesData.data.map((inquiry) => (
                    <tr key={inquiry.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors duration-150">
                      <td className="px-8 py-4 font-medium text-slate-900 dark:text-white">{inquiry.name}</td>
                      <td className="px-6 py-4 font-mono text-xs">{inquiry.email}</td>
                      <td className="px-6 py-4 capitalize">{inquiry.serviceInterested?.replace('-', ' ') || 'N/A'}</td>
                      <td className="px-6 py-4">{inquiry.budgetRange || 'N/A'}</td>
                      <td className="px-6 py-4">
                        <Badge variant="outline" className={`capitalize font-semibold text-[10px] px-2 py-0.5 rounded-full ${getStatusColor(inquiry.status)}`}>
                          {inquiry.status}
                        </Badge>
                      </td>
