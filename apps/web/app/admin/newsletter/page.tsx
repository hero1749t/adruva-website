      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-manrope">
            Manage email subscriptions and newsletter leads
          </p>
        </div>
        <Button
          ...
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              ...
              {isLoading ? (
                ...
                <p className="text-2xl font-bold font-sora text-slate-900 dark:text-white">
      ...
      <Card className="border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#151f32] shadow-sm">
        <CardHeader className="px-8 py-5 border-b border-slate-100 dark:border-slate-800/60">
          ...
          <CardDescription className="text-slate-500 dark:text-slate-400 text-xs">
            Full index of registered emails
          </CardDescription>
        </CardHeader>
              ...
              <table className="w-full text-left border-collapse text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40 text-slate-700 dark:text-slate-300">
                  {data.data.map((sub) => (
                    ...
                    >
                      <td className="px-8 py-4 font-mono font-medium text-slate-900 dark:text-white">
                        {sub.email}
                      </td>
                      <td className="px-6 py-4">
                        <Badge
                          variant="outline"
                          className={`capitalize font-semibold text-[10px] px-2 py-0.5 rounded-full ${getStatusColor(sub.status)}`}
                        >
                          {sub.status}
                        </Badge>
                      </td>
