...
const getProcessSteps = (slug: string) => {
  if (slug === 'web-development') {
    return [
      { number: '01', title: 'Discovery Call', desc: 'Understand your business goals, target audience, and competitors.' },
      { number: '02', title: 'Wireframe & Design', desc: 'Design low-fi wireframes followed by high-fidelity Figma mockups for your approval.' },
      { number: '03', title: 'Development', desc: 'Build the actual responsive website from scratch, mobile-first.' },
      { number: '04', title: 'Testing', desc: 'Rigorous cross-browser, mobile compatibility, speed, and form submission testing.' },
      { number: '05', title: 'Launch', desc: 'Deploy files, configure domains, set up SSL, and go live 🚀.' }
    ];
  }
  if (slug === 'mobile-app-development') {
    return [
      { number: '01', title: 'Discovery', desc: 'Understand target users, core features list, and map out user journeys.' },
      { number: '02', title: 'Design', desc: 'Wireframes + platform-specific visual UI design (iOS & Android guidelines).' },
      { number: '03', title: 'Development', desc: 'Build the mobile app frontend in React Native + configure backend APIs.' },
      { number: '04', title: 'Testing', desc: 'Active testing on physical devices (iOS + Android), checking performance & edge cases.' },
      { number: '05', title: 'Launch', desc: 'Coordinate full Apple App Store and Google Play Store submissions.' }
    ];
  }
  if (slug === 'google-ads') {
    return [
      { number: '01', title: 'Audit', desc: 'Perform competitor analysis and audit existing ad account setup.' },
      { number: '02', title: 'Strategy', desc: 'Compile keyword maps, write bidding models, and structure budget plans.' },
      { number: '03', title: 'Setup', desc: 'Build ad campaigns, write targeted headlines, and configure conversion tracking.' },
      { number: '04', title: 'Launch', desc: 'Deploy campaigns live, monitoring initial budget flow and search terms.' },
      { number: '05', title: 'Optimize', desc: 'Perform weekly keyword optimizations, bid revisions, and send monthly reports.' }
    ];
  }
  // Generic fallback for other 11 services
  return [
    { number: '01', title: 'Discovery & Audit', desc: 'Understand your current workflows, goals, and target objectives.' },
    { number: '02', title: 'Strategy & Design', desc: 'Design plans and map out integrations or content visual drafts.' },
    { number: '03', title: 'Implementation', desc: 'Execute modifications, configure systems, or build content blocks.' },
    { number: '04', title: 'Testing & Refine', desc: 'Test automations, ad structures, or page layouts against goals.' },
    { number: '05', title: 'Handoff & Monitor', desc: 'Provide system walkthroughs and monitor performance metrics.' }
  ];
};
...
const mockProjects = [
  // Build
  {
    ...
    category: 'build',
    industry: 'Wellness',
    description: 'Booking engine and headless website integration for Dehradun\'s premium yoga studio.',
  ...
  },
  {
    ...
    category: 'build',
    industry: 'Logistics',
    description: 'Custom inventory tracker and driver portal for regional shipping provider.',
  ...
  },
  {
    ...
    description: 'AI-powered WhatsApp lead qualifier and automated property catalog matching.',
  ...
  },
  {
    ...
    category: 'automate',
    industry: 'Finance',
    description: 'Document extraction pipeline processing PDFs into accounting systems automatically.',
  ...
  // Grow
  {
    ...
    category: 'grow',
    industry: 'Tourism',
    description: 'Google and Meta ad systems driving 150+ direct booking inquiries monthly.',
  ...
  },
  {
    ...
    category: 'grow',
    industry: 'Fitness',
    description: 'Local Meta Ads campaign generating 300+ membership signups in 90 days.',
  ...
  },
  {
    ...
    category: 'grow',
    industry: 'Healthcare',
    description: 'Ranking on Page 1 for local search terms, yielding a 200% increase in patient bookings.',
  ...
  // Design
  {
    ...
    category: 'design',
    industry: 'F&B',
    description: 'Logo, typeface, and complete modern product packaging visual identities.',
  ...
  },
  {
    ...
    category: 'design',
    industry: 'Travel',
    description: 'High-fidelity Figma wireframes and prototypes for a trekking portal.',
  ...
  },
  {
    ...
    category: 'design',
    industry: 'Tech',
    description: 'A dark-mode analytics dashboard designed for developer tool readability.',
...
export function ServicePageClient({ service }: ServicePageClientProps) {
  ...
  const relatedProjects = mockProjects.filter(
    (p) => p.category.toLowerCase() === service.category.toLowerCase()
  ).slice(0, 3);
  ...
  return (
    <div className="w-full bg-background text-foreground transition-colors duration-300">
            ...
            <div className="lg:col-span-4 mt-8 lg:mt-0">
              <div className="p-6 rounded-2xl border border-brand-orange/20 bg-gradient-to-br from-brand-orange/5 to-transparent shadow-xl relative overflow-hidden backdrop-blur-sm">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed mb-6 font-inter">
                  Transparent, value-driven rates tailored for local business scaling. No hidden management charges.
                </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-5 flex flex-col items-start">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed font-inter max-w-md">
                We manage everything end-to-end so you do not have to juggle multiple vendors. From planning to final launches and post-deployment configurations, we cover the full cycle.
              </p>
            </div>
      ...
      {(service.category === 'build' || service.category === 'automate') && service.techStack && (
        <Section>
          <Container>
        ...
        <Container>
          <div className="max-w-xl mx-auto p-8 rounded-3xl border border-brand-orange/20 bg-card shadow-xl text-center relative overflow-hidden">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed mb-8 font-inter">
              All quotes are project-based and clear. We structure milestones so you only pay for completed deliverables. Inquire now to map your scope.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
              {relatedProjects.map((project) => (
                ...
                >
                  ...
                  <div className={cn('h-40 w-full bg-gradient-to-br flex items-center justify-center p-6 relative overflow-hidden select-none', project.gradient)}>
