  ...
  return (
    <div className="w-full min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <Section className="pt-12 pb-6 md:pt-16 md:pb-8 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[300px] bg-[radial-gradient(circle_at_center,rgba(8, 120, 249,0.04)_0%,transparent_70%)] pointer-events-none select-none z-0" />
        ...
        <Container className="relative z-10">
          ...
          <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
            ...
            {/* Title */}
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-brand-navy dark:text-white mb-6 font-sora">
              Our Work
            </h1>
        ...
        <Container>
          <div className="flex flex-col gap-6 p-6 rounded-2xl border border-slate-200/80 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 max-w-4xl mx-auto mb-12">
            ...
            <div className="flex flex-col gap-2">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest font-space">
                Filter by Category
              </span>
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative px-3 py-1.5 text-xs font-semibold rounded-lg font-space transition-colors duration-300 focus:outline-none",
                        isActive
                          ? "text-white bg-brand-blue shadow-[0_2px_8px_rgba(8, 120, 249,0.2)]"
              ...
              {filteredProjects.map((project) => (
                ...
                >
                  ...
                  <div className="h-48 w-full relative overflow-hidden select-none">
                    ...
                    {/* Industry label */}
                    <div className="absolute top-4 left-4 text-[10px] font-extrabold tracking-widest text-white/90 uppercase font-space bg-black/30 px-2.5 py-0.5 rounded backdrop-blur-sm z-10">
                      {project.industry} Case Study
                    </div>

                    {/* Category tag */}
                    <div className="absolute bottom-4 left-4 flex gap-1.5 z-10">
                      <span className="text-[10px] font-bold text-white bg-brand-blue px-2.5 py-1 rounded-full uppercase tracking-wider font-space shadow-sm">
                        {project.category}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-6 flex-grow flex flex-col justify-between">
                    <div>
                      <h3 className="text-base font-semibold text-brand-navy dark:text-white mb-2 font-sora leading-snug group-hover:text-brand-blue transition-colors">
                        {project.title}
                      </h3>
                    ...
                    <div className="mt-auto space-y-4">
                      <div className="flex flex-wrap gap-1.5">
                        {project.techStack.slice(0, 3).map((tech) => (
                          <span
                            key={tech}
                            className="bg-muted text-muted-foreground text-xs font-semibold px-2.5 py-0.5 rounded-full border border-border/10 font-manrope"
                          >
                            {tech}
                          </span>
                        ))}
          ...
          {filteredProjects.length === 0 && (
            <div className="text-center py-16 p-6 rounded-2xl border border-dashed border-border/60 bg-muted/5 max-w-md mx-auto mt-6">
              <span className="text-xs font-semibold text-muted-foreground font-manrope">
